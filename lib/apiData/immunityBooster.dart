    ...
    try {
      ...
      for (int i = 0; i < immunityBoosterProduct.length; i++) {
        ...
        for (int j = 0; j < immunityBoosterProductPrice.length; j++) {
          ...
          newOldPrice.add("${immunityBoosterProductPrice[j].pId}");
          newOldPrice.add(
              '${immunityBoosterProductPrice[j].weight} ${immunityBoosterProductPrice[j].unit}');
          newOldPrice.add('${immunityBoosterProductPrice[j].originalPrice}');
          newOldPrice.add('${immunityBoosterProductPrice[j].discountedPrice}');
          newOldPrice.add('${immunityBoosterProductPrice[j].unit}');
        ...
        if (immunityBoosterProductPrice[0].discountedPrice == "")
          ...
          newRate =
              double.parse(immunityBoosterProductPrice[0].discountedPrice);

        immunityBoosterList.add(ImmunityBoosterCategories(
            imageUrl: immunityBoosterProduct[i].img,
            title: immunityBoosterProduct[i].name,
            newrate: newRate,
            oldrate: double.parse(immunityBoosterProductPrice[0].originalPrice),
            weight: immunityBoosterProductPrice[0].weight,
            description: immunityBoosterProduct[i].description,
            data: manyNewOldPrice,
            productId: immunityBoosterProduct[i].id));
      }
    } catch (e) {
      print(e);
    }
