...
Future<void> fetchGetUserFav(String userId) async{
  print("$header/app_api/getUserFavProduct.php?user_id=$userId&user_type=Society");

  http.Response response =await http.get(Uri.parse("$header/app_api/getUserFavProduct.php?user_id=$userId&user_type=Society"));
  ...
  if(response.statusCode==200) {
    ...
    for (int i = 0; i < favProducts.length; i++) {
      ...
      for (int j = 0; j < favProductsPrice.length; j++) {
        ...
        newOldPrice.add(
            '${favProductsPrice[j].weight} ${favProductsPrice[j].unit}');
      ...
      userFavProductList.add(UserFavProductCategories( title: favProducts[i].name, weight : manyNewOldPrice,rate: newRate.toInt(),imageURL: favProducts[i].img,categoryName: favProducts[i].categoryName,discount: favProductsPrice[0].discount));
