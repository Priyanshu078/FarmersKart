       ...
       List<List> manyNewOldPrice=[];
           ...
           for(int j=0;j<BestProductsPrice.length;j++){
             ...
             List<String> newOldPrice=[];
              newOldPrice.add('${BestProductsPrice[j].weight} ${BestProductsPrice[j].unit}');
           ...
           bestProductCategoryList.add(BestProductCategories(imageUrl: BestProducts[i].img, title: BestProducts[i].name, newrate: newRate, oldrate:double.parse(BestProductsPrice[0].originalPrice), weight: BestProductsPrice[0].weight, description: BestProducts[i].description, data: manyNewOldPrice,productId: BestProducts[i].id));
     }
...
enum Subcategory { FRESH_VEGITABLES, FRUITS, PEELED_OR_CUT_VEGITABLES, LEAFY_VEGETABLES }
